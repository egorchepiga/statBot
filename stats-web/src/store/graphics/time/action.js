...
export const createTimeMessage = (timeArray, dayScale = '0', imposition = false, fromTime = 0, toTime = 0, timeScale = '0', average = false, periods = 1) =>
    dispatch => {
        ...
        for (let i = 0; i < preparedTimeArrays.length; i++) {
            ...
            for (let time in preparedTimeArrays[i])
                timeGraphicData[i].push({t: time, y: preparedTimeArrays[i][time]});       //наносим метки на Ox и Oy
        }
...
function prepareTime(arr, dayScale, imposition, fromTime, toTime, timeScale, average, periods) {
    ...
    switch (dayScale) {
        ...
        default:                                                                            //всё время
            ...
            let label = monthDays.call(arr[0]) + " - " + monthDays.call(timeToShow);
    ...
    for (let i = 0; i < timeArray.length; i++) {
        timesWithoutNaN.push({});
        timesWithoutNaN[i]['label'] = timeArray[i]['label'];
        for (let key in timeArray[i]) {
            if (!isNaN(timeArray[i][key]))
                timesWithoutNaN[i][key] = timeArray[i][key];
        }
    }*/
    return timeArray;
}
...
function scaleTimeGraphic(arr, func, timeFromShow, timeToShow = 0) {
    ...
    date.setSeconds(0);
    while (true) {                                               //Первая запись массива времени < последнее учитываемое время за день
        if (tmpTimeFromShow < date) {
            tmpTimeFromShow.setHours(hours);
            hours = tmpTimeFromShow.getHours() + 1;                                    //Увеличиваем счётчик первой записи на 1 час вперёд
            let str = func.call(tmpTimeFromShow);
            console.log(str);
            placeholder[str] = 0;
        } else break;                                               //функция-шаблон {time:'XXXX-XX/XX/XX', ...} -> {'XX.XX XX:XX' : 0, ...)
    ...
    for (let i = arr.length-1; i > -1; i--)                                        //Фильтруем массив времени по дате с конца
        ...
        else times.push(new Date(arr[i]));
...
function createObjForReducer(timeArray, preparedTimeArray, timeGraphicData, dayScale, timeScale, fromTime, toTime, imposition, average, periods) {
    ...
    for (let i = 0; i < timeGraphicData.length; i++) {
        B = B < R ? B + 30 : B;
        R = B > R ? R - 30 : R;
        let label = timeGraphicData[i][0].y;
        delete preparedTimeArray[i].label;
        timeGraphicData[i].splice(0,1);
        dataSets.push({
            label: label,
...
let sixHours = function () {                                                    // XXXX-XX/XX/XX XX:XX -> XX.XX 03:00, 09:00...
    let hour = this.getHours(),
        strHour = '';
    if (hour <= 6) strHour = "03";
    else if (hour <= 12) strHour = "09";
...
let days = function () {                                                      // XXXX-XX/XX/XX XX:XX -> XX.XX
